...
module.exports = async (req, res) => {
  ...
  try {
    const { league: leagueGr='ΑΓΓ1', home: homeTeam='', away: awayTeam='' } = req.query;
    const currentSeason = '2425';
    const prevSeason    = '2324';

    const leagueCode = LEAGUE_CODES[leagueGr];
    if (!leagueCode) return res.status(400).json({ error: `Άγνωστη λίγκα: ${leagueGr}` });
    ...
    if (!rowsCurrent.length) return res.status(404).json({ error: 'Δεν βρέθηκαν δεδομένα' });

    // Προηγούμενη σεζόν για H2H
    let rowsPrev = [];
    try {
      const urlPrev = `https://www.football-data.co.uk/mmz4281/${prevSeason}/${leagueCode}.csv`;
      rowsPrev = parseCSV(await fetchCSV(urlPrev));
    } catch(e) {}
